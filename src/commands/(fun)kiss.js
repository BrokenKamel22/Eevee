...
module.exports = new Command({
    ...
    category: 'FUN',
    version: '0.1.0',
    description: "Give a kiss to someone who deserves it.",
    usage: 'kiss [@user]',
    ...
    async run(message, args, client) {
        ...
        // Obtaining kissing media from neko-love API
        const { data } = await axios('https://neko-love.xyz/api/v1/kiss').catch(async err_1a => {
            console.error(`-> kiss-1a: ${err_1a}`.red);
            ...
            return message.channel.send({ embeds: [unknownErrorEmbed] }).catch(err_1b => {
                return console.error(`-> kiss-1b: ${err_1b}`.red);
            });
        });

        // Sending the response to event channel
        try {
            const kissingEmbed = new MessageEmbed()
            .setColor(styles.color.fun)
            .setImage(data.url);

            return message.channel.send({
                content: user === message.author
                    ? `${user} kissed themselves because they are lonely :C`
                    : `${message.author} gave a kiss to ${user}!`,
                embeds: [kissingEmbed]
            });
        } catch(err_2a) {
            console.error(`-> kiss-2a: ${err_2a}`.red);
            ...
            return message.channel.send({ embeds: [unknownErrorEmbed] }).catch(err_2b => {
                return console.error(`-> kiss-2b: ${err_2b}`.red);
            });
        }
    }
});
