...
module.exports = new Command({
    ...
    category: 'FUN',
    version: '0.1.0',
    description: "Displays a random neko (SFW) for your enjoyment.",
    usage: 'neko',

    async run(message, args, client) {

        // Obtaining neko media from neko-love API
        const { data } = await axios('https://neko-love.xyz/api/v1/neko').catch(async err_1a => {
            console.error(`-> neko-1a: ${err_1a}`.red);
            ...
            return message.channel.send({ embeds: [unknownErrorEmbed] }).catch(err_1b => {
                return console.error(`-> neko-1b: ${err_1b}`.red);
            });
        });
        ...
        try {
            ...
            console.error(`-> neko-2a: ${err_2a}`.red);
            ...
            return message.channel.send({ embeds: [unknownErrorEmbed] }).catch(err_2b => {
                return console.error(`-> neko-2b: ${err_2b}`.red);
            });
        }
    }
});
